...
namespace Homework5_1
{
    class OrderService
    {
        ...
        public List<Order> OrderByGoodsName(Good goodsName)
        {
            var query = orders.Where(order => order.orderDetails.Exists(item => item.good == goodsName)).OrderBy(o => o.price);
            return query.ToList();
        }
        public List<Order> OrderByCustomerName(Customer cname)
        {
            var query = orders.Where(order => order.customer == cname).OrderBy(o => o.price).ToList();
            return query;
        }
        public void ShowAll()
        {
